...
job_placement = pd.read_csv("data/Placement_Data_Full_Class.csv")
...
# Yes, realistically this dataset can address my problem from an analytical perspective, as I can use the data to compare the graduation rates of HBCUs and non-HBCUs. 
# However, from a predictive perspective, this dataset is not ideal. The main reason for this is that the prevalence of HBCUs in the dataset is around 2-3%, meaning 
# that any model would have to be almost perfect to be useful. Additionally, the problem of comparing graduation rates between HBCUs and non-HBCUs isn't really one 
# that a model can predict, so any model would be an analytical tool instead of a predictive one.

# 2) What areas or variables concern you?

# The main area of concern for me is that the dataset has an hbcu prevalence of around 2-3%, meaning that any model would have to be almost perfect to be useful. 

# 3) What limitations or risks do you see?

# The main limitation is moreso that there aren't actually any limitations, rather issues with model building. DUe to the low prevalence of HBCUs in the dataset, 
# any model would have to be almost perfect to be useful. Additionally, the problem of comparing graduation rates between HBCUs and non-HBCUs isn't really one 
# that a model can predict, so any model would be an analytical tool instead of a predictive one. Additionally,  I chose a problem that really can't be predicted by a model. It is more analytical than predictive.

# Job Placement:

# 1) Can this dataset realistically address your problem?

# Yes, it can be used to address the problem from both an analytical and predictive perspective. I can use the data to compare the salaries of men and women in the same degree type, or
# build a model to predict salary.

# 2) What areas or variables concern you?

# My main worry is the the degree type variable is very broad, and there could be an illusion of equivalence where none exists if women and men trend towards different majors within the same type of degree.

# 3) What limitations or risks do you see?

# The main limitation is the size. It is pretty small and only has data from a single university, so it could lead to false conclusions about the true relationship between the earnings of men and 
# women in similar fields. Also the fear of illusion of equivalence where none exists.
# %%
